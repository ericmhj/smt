...
-- Enums
DO $$ BEGIN
  CREATE TYPE user_role AS ENUM ('superusuario', 'admin', 'manager', 'tecnico', 'tecnico_de_campo');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
  CREATE TYPE reactivo_state AS ENUM ('pendiente', 'en_revision', 'validado', 'rechazado', 'finalizado');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
  CREATE TYPE signature_type AS ENUM ('upload', 'canvas');
EXCEPTION WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE IF NOT EXISTS reactivos (
  ...
  state reactivo_state NOT NULL DEFAULT 'pendiente',
...
RETURNS TRIGGER AS $$
BEGIN
  RAISE EXCEPTION 'audit_logs table is append-only. UPDATE and DELETE are not allowed.';
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
